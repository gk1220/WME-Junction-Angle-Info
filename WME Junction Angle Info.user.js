...
// @downloadURL https://update.greasyfork.org/scripts/35547/WME%20Junction%20Angle%20Info.user.js
// @updateURL https://update.greasyfork.org/scripts/35547/WME%20Junction%20Angle%20Info.meta.js
// ==/UserScript==
...
function run_ja() {
	 ...
	 */
    var junctionangle_version = "2.2.8";
    ...
    function testSelectedItem(){
        ...
        window.W.selectionManager.getSelectedFeatures().forEach(function(element) {
            switch (element.attributes.wazeFeature._wmeObject.type) {
                case "node":
                case "segment":
...
*/			//Beta editor
    ...
    function createToggler(){
        ...
        oldTogglers.forEach(function(elt,idx){
            if(elt.id != "toolboxUl"){
                ...
                newToggler.innerHTML = '<div class="controls-container toggler"><input class="layer-switcher-item_junction_angles toggle" id="layer-switcher-item_junction_angles" type="checkbox"><label for="layer-switcher-item_junction_angles"><span class="label-text">'+ja_getMessage("name")+'</span></label></div>';
    ...
    function ja_calculate_real() {
        ...
        window.W.selectionManager.getSelectedFeatures().forEach(function(element) {
            ...
            switch (element.attributes.wazeFeature._wmeObject.type) {
                case "node":
                    ja_nodes.push(element.attributes.wazeFeature._wmeObject.attributes.id);
                    break;
                case "segment":
                    //segments selected?
                    if (element.attributes.wazeFeature._wmeObject.attributes.fromNodeID != null &&
                        ja_nodes.indexOf(element.attributes.wazeFeature._wmeObject.attributes.fromNodeID) === -1) {
                        ja_nodes.push(element.attributes.wazeFeature._wmeObject.attributes.fromNodeID);
                    }
                    if (element.attributes.wazeFeature._wmeObject.attributes.toNodeID != null &&
                        ja_nodes.indexOf(element.attributes.wazeFeature._wmeObject.attributes.toNodeID) === -1) {
                        ja_nodes.push(element.attributes.wazeFeature._wmeObject.attributes.toNodeID);
                    }
                    break;
                case "venue":
                    break;
                default:
                    ja_log("Found unknown item type: " + element.attributes.wazeFeature._wmeObject.type, 2);
                    break;
            }
            ja_log(ja_nodes, 2);
        });
        ...
        if (ja_getOption("angleMode") === "aDeparture" && ja_nodes.length > 1) {
            window.W.selectionManager.getSelectedFeatures().forEach(function (selectedSegment) {
                var segmentId = selectedSegment.attributes.wazeFeature._wmeObject.attributes.id;
        ...
        for (var i = 0; i < ja_nodes.length; i++) {
            ...
            ja_log(ja_current_node_segments, 3);
            window.W.selectionManager.getSelectedFeatures().forEach(function (selectedSegment) {
                var selectedSegmentId = selectedSegment.attributes.wazeFeature._wmeObject.attributes.id;
